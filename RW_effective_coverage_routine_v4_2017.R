####-----------------------------------------------------------####
####-----------------------------------------------------------####
#   The program recodes DHS/MIS survey variables related to fever,
#   health-seeking, and drug treatments to derive malaria effective
#   coverage metrics by country, admin1, and survey.
#   
#   Requires the following files
#   - RX recodes: dhsrecode_rx_hbhi.csv
#   - HSB recodes: dhsrecode_hsb_hbhi.csv
#   - DHS data
#   
#   Output: dhs_effcov_hbhi_allSrvy_v4.csv
#   
#   Created by: Katya Galactionova
#   e.galactionova@unibas.ch
#   
#   Converted from Stata to R by Rémi Turquier
#   remi.turquier@ensae.fr
#
...
#' Use the `rdhs` library to download / import DHS data 
#' directly from the R program. This way, the data would be
#' automatically kept up to date.
#' See this blog post:
#' www.r-bloggers.com/interacting-with-the-demographic-and-health-surveys-dhs-program-data
...
setwd("C:/Users/Martin Habimana/Desktop/Other MIS data")

#' Set paths to the two mapping files, to the DHS directory and to 
#' the output directory.

dhs_dir    <- "C:/Users/Martin Habimana/Desktop/Other MIS data"
hsb_path   <- "C:/Users/Martin Habimana/Desktop/Other MIS data/children_access_newvariables_2017.csv"
rx_path    <- "C:/Users/Martin Habimana/Desktop/Other MIS data/children_medicines_newvariables_2017.csv"
dhs_file   <- "C:/Users/Martin Habimana/Desktop/Other MIS data/RWKR7AFL.DTA"
output_dir <- "C:/Users/Martin Habimana/Desktop/Other MIS data/eff_cov_output_2017"


#' The DHS directory contains one directory for each country (eg. `…/DHS/Benin`)
#' and each country directory contains .dta files corresponding to DHS surveys.

# List of countries of interest
#countries <-
#  c("Benin", "Cameroon", "Ghana", "Haiti", "Madagascar", "Mozambique",
#    "Namibia", "Tanzania", "Uganda", "Zimbabwe")

countries <- c("Rwanda")

# Set the period over which metrics should be calculated
#' `year_range` can be a list of years, or a single year. 
...
survey_metrics     <- function(df){
  #' Takes a recoded DHS dataframe, summarises it, and 
  #' calculates effective coverage. 
  #' Returns a summarised dataframe with effective coverage 
  #' at admin1 and country level.
  ...
  #' Create `admin_level`, which is either `"country"` or the name of the
  #' admin1 units in the country (eg. `"region"`)
  admin1_mean[["admin_level"]] <- 
    attr(admin1_mean[["admin1"]], "label") %>%
    tolower()
  
  #' Save numbers identifying admin1s to variable `admin1_num`.
  #' This might by useful to get around typos / spelling errors when working
  #' at admin1 level.
  ...
  metrics_df <- 
    ...
    mutate(
      ...
      # aggregate provider categories to map into WHO-CHOICE
...
effective_coverage <- function(access, formal, compl_f, compl_i){
  ...
  if_else(
    access == 0,
    0,
    access * ( formal * (compl_f * adh_act_f * cure_act_f + (1 - compl_f) * cure_othrantimal_f)
    + (1 - formal) * (compl_i * adh_act_i * cure_act_i + (1 - compl_i) * cure_othrantimal_i)
    )
  )
}
...
#' These functions can be called *after* running the routine
#' to create plots of the effective coverage routine.

latest_year    <- function(country_name){
  #' Call by the `plot_*` fucntions
...
plot_waterfall <- function(waterfall_country, waterfall_year = NULL){
  ...
  adh_act_f          <- 0.8
  adh_act_i          <- adh_act_f * 0.8
  adh_othrantimal_f  <- 1    #? Check with Katya what is the assumed adherence 
  adh_othrantimal_i  <- 1    #? for second line antimalarials
...
plot_bars      <- function(bars_country, bars_year = NULL){
  #' Not called
  #' Can be called once the routine has been run.
  #' 
...
# export to csv
output_path <- file.path(output_dir, "dhs_effcov_hbhi_allSrvy_v4.csv")
